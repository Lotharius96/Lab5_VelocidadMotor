...
void Keypad_1_Clock_Start(void) 
{
    ...
    Keypad_1_Clock_CLKEN |= Keypad_1_Clock_CLKEN_MASK;
	Keypad_1_Clock_CLKSTBY |= Keypad_1_Clock_CLKSTBY_MASK;
}
...
void Keypad_1_Clock_Stop(void) 
{
    ...
    Keypad_1_Clock_CLKEN &= (uint8)(~Keypad_1_Clock_CLKEN_MASK);
	Keypad_1_Clock_CLKSTBY &= (uint8)(~Keypad_1_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Keypad_1_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_StopBlock(void) 
{
    if ((Keypad_1_Clock_CLKEN & Keypad_1_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(Keypad_1_Clock__CFG3)
        CLK_DIST_AMASK = Keypad_1_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Keypad_1_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Keypad_1_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Keypad_1_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Keypad_1_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Keypad_1_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Keypad_1_Clock_CLKSTBY &= (uint8)(~Keypad_1_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Keypad_1_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Keypad_1_Clock_GetSourceRegister();
    uint16 oldDivider = Keypad_1_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Keypad_1_Clock_CLKEN & Keypad_1_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Keypad_1_Clock_DIV_PTR, clkDivider);
                Keypad_1_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Keypad_1_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Keypad_1_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Keypad_1_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Keypad_1_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Keypad_1_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_SetModeRegister(uint8 modeBitMask) 
{
    Keypad_1_Clock_MOD_SRC |= modeBitMask & (uint8)Keypad_1_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Keypad_1_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    Keypad_1_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Keypad_1_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Keypad_1_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Keypad_1_Clock_GetModeRegister(void) 
{
    return Keypad_1_Clock_MOD_SRC & (uint8)(Keypad_1_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Keypad_1_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Keypad_1_Clock_GetDividerRegister();
    uint8 oldSrc = Keypad_1_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Keypad_1_Clock_MOD_SRC |= CYCLK_SSS;
        Keypad_1_Clock_MOD_SRC =
            (Keypad_1_Clock_MOD_SRC & (uint8)(~Keypad_1_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Keypad_1_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Keypad_1_Clock_GetSourceRegister(void) 
{
    return Keypad_1_Clock_MOD_SRC & Keypad_1_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Keypad_1_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Keypad_1_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    Keypad_1_Clock_PHASE = clkPhase & Keypad_1_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Keypad_1_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Keypad_1_Clock_GetPhaseRegister(void) 
{
    return Keypad_1_Clock_PHASE & Keypad_1_Clock_PHASE_MASK;
}
